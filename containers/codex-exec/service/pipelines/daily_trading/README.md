...
- Market index snapshot은 optional best-effort run-level 입력이다. 해당 데이터 부재만으로 verdict 또는 주문 실행을 차단하지 않는다.
