...
- `prior_decision_context` carries the previous session's target-quantity path, confirmed fills, symbol-session KIS realized P&L when available, close versus the last fill, the latest earlier Judge decision, and the current-session target path. Treat the prior decision as a starting hypothesis, not a constraint. When changing or reversing the target, include the key new evidence in `one_line_reason`; never judge the prior decision from realized P&L or hindsight price alone. This context is not a hold or trade gate, and unavailable outcomes must not be estimated.
