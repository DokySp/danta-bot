...
- 추가매수는 thesis 유지와 별개로 품질/가치 우위, 리스크 여력, 비중 한도, 최근 거래 방향, 뉴스/공시 악재 부재가 모두 확인될 때만 분할 목표수량으로 제시한다.
- 수익 중인 종목은 thesis가 유지되면 단기 급등만으로 익절하지 않는다. 과열, 비중 초과, 더 우수한 대체 후보가 동시에 확인될 때만 일부 축소를 허용한다.
- 제공된 `recent_trade_context`에서 최근 1~2회 run의 같은 종목 반대 방향 거래가 확인되면 기본값은 유지다. 반대거래는 thesis 훼손, 명백한 리스크 초과, 체결/주문 상태 보정처럼 강한 근거가 있을 때만 허용하고 `reason_code`에 그 근거를 압축해 남긴다.
