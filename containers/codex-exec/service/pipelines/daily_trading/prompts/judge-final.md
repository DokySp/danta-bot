...
- 점수는 자동 주문 규칙이 아니라 목표수량 판단 입력이다. 포트폴리오 비중, 중복 노출, active order, 급격한 가격/수급 변화 등을 함께 고려해 목표수량을 정한다.
...
- 추가매수는 thesis 유지와 별개로 품질/가치 우위, 리스크 여력, 비중 한도, 당일/최근 거래 맥락의 명시적 재평가, 뉴스/공시 악재 부재가 모두 확인될 때만 분할 목표수량으로 제시한다.
