...
def render_time_symbol_inspector(runs: list[dict[str, Any]], fills: list[dict[str, Any]]) -> str:
    ...
    for run_index, run in enumerate(runs):
        ...
        time_buttons.append(
            f'<button type="button" aria-pressed="{str(is_active_time).lower()}" class="time-button{" active" if is_active_time else ""}" data-time-target="{esc(time_key)}">'
            f'<strong>{run_index + 1}회차 · {esc(run_time)}</strong>'
            f'<small>신규 {time_button_new_count} · 정정·취소 {time_button_correction_count + time_button_cancellation_count} · 체결 {len(linked_fills)} · 차단 {len(blocked_orders)}</small></button>'
        )
        ...
        for analyst_item in analyst_symbols:
            ...
            selector_decision = (
                judge_field_display(final_item, "canonical_action", CANONICAL_ACTION_LABELS)
                if final_item
                else {
                    "unresolved_in_scope": "미해결",
                    "not_selected": "미선정",
                    "legacy_unknown": "상태 미기록",
                }[judge_scope_status]
            )
    ...
    <section class="panel" id="trade-symbol-analysis">
      <div class="section-head"><div><p class="kicker">RUN &amp; SYMBOL FOCUS</p><h2>회차별 거래·전체 종목 판단</h2></div><span class="badge info">회차 → 종목 순서로 선택</span></div>
      <p class="section-note">회차를 선택하면 해당 run의 주문과 주문번호로 연결된 체결, Analyst 대상 종목 전체를 표시합니다. Judge 미진입 종목도 Analyst 상세 점수를 확인할 수 있습니다.</p>
      <div class="time-wheel"><span class="time-wheel-caption">실행 회차</span><div class="time-selector" role="group" aria-label="실행 회차">{''.join(time_buttons)}</div></div>
    ...
    *,
    ...
    <section class="combined-chart-card" data-chart-points="{points_json}" data-chart-period="{esc(period)}">
      ...
      <div class="chart-scrubber">
        <div class="chart-scrubber-label"><span class="chart-scrubber-copy"><strong>시점 탐색</strong><small>차트나 트랙을 드래그하세요</small></span><output class="chart-scrubber-time">{esc(rows[-1]['time'])}</output></div>
...
def render_symbol_history_chart(points: list[dict[str, Any]], period: str) -> str:
    ...
    left, right, top, bottom = 112, 30, 28, 54
    plot_width = width - left - right
    plot_height = height - top - bottom
    ...
    points_json = esc(
        json.dumps(
            [{"label": str(point.get("label") or "")} for point in points],
            ensure_ascii=False,
            separators=(",", ":"),
        )
    )
    ...
    price_points = "".join(
        f'<circle class="symbol-price-point" data-point-index="{index}" cx="{xs[index]:.2f}" cy="{price_y_by_index[index]:.2f}" r="4">'
    ...
    quantity_points = "".join(
        f'<circle class="symbol-quantity-point" data-point-index="{index}" cx="{xs[index]:.2f}" cy="{quantity_y_by_index[index]:.2f}" r="4">'
    ...
    return f"""
    <section class="symbol-history-chart" data-symbol-chart-points="{points_json}">
      ...
      <svg viewBox="0 0 {width} {height}" role="img" aria-label="{esc(period_label)} 가격과 보유수량 추이">
        {grid}
        <text x="12" y="{top + 5}" class="symbol-axis-label symbol-price-axis-label">가격 (원)</text>
        <text x="12" y="{top + 27}" class="symbol-axis-label symbol-quantity-axis-label">보유수량 (주)</text>
      ...
      <div class="chart-scrubber">
        <div class="chart-scrubber-label"><span class="chart-scrubber-copy"><strong>시점 탐색</strong><small>트랙을 드래그하세요</small></span><output class="chart-scrubber-time">{esc(points[-1].get('label'))}</output></div>
        <input class="chart-range-slider" type="range" min="0" max="{len(points) - 1}" step="1" value="{len(points) - 1}" aria-label="{esc(period_label)} 가격·보유수량 조회 시점" aria-valuetext="{esc(points[-1].get('label'))}">
        <div class="chart-scrubber-ends"><span>{esc(points[0].get('label'))}</span><span>{esc(points[-1].get('label'))}</span></div>
      </div>
...
<script>
  (() => {{
    ...
    document.querySelectorAll('.combined-chart-card').forEach((card) => {{
      ...
      if (slider) {{
        slider.addEventListener('input', () => showPointAtIndex(Number(slider.value)));
        slider.addEventListener('pointerup', hidePoint);
        slider.addEventListener('pointercancel', hidePoint);
        slider.addEventListener('blur', hidePoint);
      }}
    }});

    document.querySelectorAll('.symbol-history-chart[data-symbol-chart-points]').forEach((chart) => {{
      const points = JSON.parse(chart.dataset.symbolChartPoints || '[]');
      const slider = chart.querySelector('.chart-range-slider');
      const sliderValue = chart.querySelector('.chart-scrubber-time');
      const chartPoints = [...chart.querySelectorAll('[data-point-index]')];
      if (!points.length || !slider || !sliderValue) return;
      const selectPoint = () => {{
        const index = Math.max(0, Math.min(points.length - 1, Number(slider.value)));
        const label = points[index].label;
        sliderValue.textContent = label;
        slider.setAttribute('aria-valuetext', label);
        chartPoints.forEach((point) => point.classList.toggle('selected', Number(point.dataset.pointIndex) === index));
      }};
      slider.addEventListener('input', selectPoint);
      selectPoint();
    }});
    ...
    document.querySelectorAll('.portfolio-pie').forEach((chart) => {{
      ...
      slices.forEach((slice) => {{
        ...
        slice.addEventListener('pointerup', (event) => {{ if (event.pointerType !== 'mouse') hideSlice(); }});
        slice.addEventListener('pointercancel', hideSlice);
        slice.addEventListener('pointerleave', hideSlice);
        slice.addEventListener('focus', () => {{ if (slice.matches(':focus-visible')) showSlice(slice); }});
