...
class RenderHtmlReportSelfTest(unittest.TestCase):
    ...
    def test_cumulative_report_hides_touch_overlays_and_styles_the_scrubber(self) -> None:
        ...
        self.assertIn("document.querySelectorAll('.symbol-history-chart[data-symbol-chart-points]')", rendered)
        self.assertIn("slider.addEventListener('input', selectPoint);", rendered)
    ...
    def test_symbol_history_chart_renders_price_and_step_holding_series(self) -> None:
        ...
        self.assertIn('class="symbol-axis-label symbol-price-axis-label">가격 (원)</text>', rendered)
        self.assertIn('class="symbol-axis-label symbol-quantity-axis-label">보유수량 (주)</text>', rendered)
        self.assertIn('class="chart-range-slider"', rendered)
        self.assertIn('data-point-index="0"', rendered)
