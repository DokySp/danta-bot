...
from ..scripts.collect_main_evidence import (
    ...
    summarize_orderbook,
    valid_reused_account_artifact,
    valid_reused_today_fills_artifact,
)
...
class ReuseAccountArtifactValidationTest(unittest.TestCase):
    """valid_reused_{account,today_fills}_artifact gate the full-review reuse
    contract: only an exact-run_id, fully successful (non-partial), correctly
    scoped artifact may be reused instead of re-collected.
    """

    def test_valid_account_artifact_is_accepted(self) -> None:
        self.assertTrue(
            valid_reused_account_artifact({"run_id": "r", "status": "success", "symbols": []}, run_id="r")
        )

    def test_partial_status_account_artifact_is_rejected(self) -> None:
        self.assertFalse(
            valid_reused_account_artifact({"run_id": "r", "status": "partial", "symbols": []}, run_id="r")
        )

    def test_wrong_run_id_account_artifact_is_rejected(self) -> None:
        self.assertFalse(
            valid_reused_account_artifact({"run_id": "other", "status": "success", "symbols": []}, run_id="r")
        )

    def test_missing_symbols_account_artifact_is_rejected(self) -> None:
        self.assertFalse(valid_reused_account_artifact({"run_id": "r", "status": "success"}, run_id="r"))

    def test_missing_account_artifact_is_rejected(self) -> None:
        self.assertFalse(valid_reused_account_artifact(None, run_id="r"))
        self.assertFalse(valid_reused_account_artifact({}, run_id="r"))

    def test_skipped_account_artifact_is_rejected(self) -> None:
        # skipped_account_artifact() intentionally reports status="success"
        # with skipped=True and an empty account_summary; it must never be
        # accepted as a genuine full collection to reuse.
        self.assertFalse(
            valid_reused_account_artifact(
                {"run_id": "r", "status": "success", "skipped": True, "symbols": []}, run_id="r"
            )
        )

    def test_valid_today_fills_artifact_is_accepted(self) -> None:
        self.assertTrue(
            valid_reused_today_fills_artifact(
                {"run_id": "r", "status": "success", "skipped": False, "fill_scope": "account", "fills": []}, run_id="r"
            )
        )

    def test_partial_status_today_fills_artifact_is_rejected(self) -> None:
        self.assertFalse(
            valid_reused_today_fills_artifact(
                {"run_id": "r", "status": "partial", "skipped": False, "fill_scope": "account", "fills": []}, run_id="r"
            )
        )

    def test_wrong_run_id_today_fills_artifact_is_rejected(self) -> None:
        self.assertFalse(
            valid_reused_today_fills_artifact(
                {"run_id": "other", "status": "success", "skipped": False, "fill_scope": "account", "fills": []}, run_id="r"
            )
        )

    def test_skipped_today_fills_artifact_is_rejected(self) -> None:
        self.assertFalse(
            valid_reused_today_fills_artifact(
                {"run_id": "r", "status": "success", "skipped": True, "fill_scope": "account", "fills": []}, run_id="r"
            )
        )

    def test_wrong_scope_today_fills_artifact_is_rejected(self) -> None:
        self.assertFalse(
            valid_reused_today_fills_artifact(
                {"run_id": "r", "status": "success", "skipped": False, "fill_scope": "universe", "fills": []}, run_id="r"
            )
        )

    def test_missing_today_fills_artifact_is_rejected(self) -> None:
        self.assertFalse(valid_reused_today_fills_artifact(None, run_id="r"))
        self.assertFalse(valid_reused_today_fills_artifact({}, run_id="r"))


class CommandCollectReuseAccountFailureTest(unittest.TestCase):
    """command_collect --reuse-account must fail the whole command (nonzero)
    when the reused artifact is invalid, never merely degrade to "partial"
    just because price collection itself succeeded.
    """
    ...
    def run_command_collect(self) -> int:
        args = argparse.Namespace(
            ...
            skip_account=False,
            skip_price_chart=False,
            skip_account_asset=True,
            reuse_account=True,
            skip_extended_market_evidence=True,
            retries=1,
            max_account_pages=1,
        )
        with (
            patch("service.pipelines.daily_trading.scripts.collect_main_evidence.kis_credentials", return_value=("key", "secret")),
            patch(
                "service.pipelines.daily_trading.scripts.collect_main_evidence.fetch_token",
                return_value=("token", "existing_token", ""),
            ),
            patch(
                "service.pipelines.daily_trading.scripts.collect_main_evidence.collect_price_chart",
                return_value={"schema_version": "1", "status": "success", "errors": [], "symbols": []},
            ),
        ):
            from ..scripts.collect_main_evidence import command_collect as command_collect_fn

            return command_collect_fn(args)

    def test_invalid_reused_account_artifact_returns_nonzero(self) -> None:
        (self.output_dir / "account-before-order.json").write_text(
            json.dumps({"run_id": "r", "status": "partial", "symbols": []}), encoding="utf-8"
        )
        (self.output_dir / "today-fills.json").write_text(
            json.dumps({"run_id": "r", "status": "success", "skipped": False, "fill_scope": "account", "fills": []}),
            encoding="utf-8",
        )
        self.assertNotEqual(self.run_command_collect(), 0)

    def test_invalid_reused_today_fills_artifact_returns_nonzero(self) -> None:
        (self.output_dir / "account-before-order.json").write_text(
            json.dumps({"run_id": "r", "status": "success", "symbols": []}), encoding="utf-8"
        )
        (self.output_dir / "today-fills.json").write_text(
            json.dumps({"run_id": "r", "status": "partial", "skipped": False, "fill_scope": "account", "fills": []}),
            encoding="utf-8",
        )
        self.assertNotEqual(self.run_command_collect(), 0)

    def test_valid_reused_artifacts_return_zero(self) -> None:
        (self.output_dir / "account-before-order.json").write_text(
            json.dumps({"run_id": "r", "status": "success", "symbols": []}), encoding="utf-8"
        )
        (self.output_dir / "today-fills.json").write_text(
        ...
        self.assertEqual(self.run_command_collect(), 0)
