...
def step_decision_brief_evidence_and_strategy_context_checks(tmp: Path, run_dir: Path) -> list[str]:
    """Decision-brief construction, financial/news evidence summaries, and strategy-context regime derivation."""
    failures: list[str] = []
    try:
        # Written to disk by run_self_test's shared fixture setup, which always runs first.
...
def step_today_trade_context_checks(tmp: Path, run_dir: Path) -> list[str]:
    """today_trade_context/today_trade_collection_context across missing/skipped/partial/failed collection states."""
    failures: list[str] = []
    try:
...
def step_first_specs_and_analyst_review_checks(tmp: Path, run_dir: Path) -> list[str]:
    ...
    try:
        ...
        if any(spec.get("review_contract_version") != 10 for spec in first_specs["specs"]):
            failures.append(f"analyst specs must use review contract version 10: {first_specs}")
...
def step_etf_and_invalid_analyst_review_checks(tmp: Path, run_dir: Path) -> list[str]:
    """ETF-aware decision-brief/analyst-review reuse plus invalid-score and missing-persona-score analyst-review handling."""
    failures: list[str] = []
    try:
        # Written to disk by step_first_specs_and_analyst_review_checks, which always runs first.
        subagent_dir = run_dir / "subagents"
        etf_dir = tmp / "reports" / "runs" / "etf-quality-probe"
        etf_portfolio_path = tmp / "etf-portfolio.json"
        etf_cache_path = tmp / "etf-financial-cache.json"
...
def step_second_spec_checks(tmp: Path, run_dir: Path) -> list[str]:
    ...
    try:
        ...
        if second_spec.get("review_contract_version") != 10:
            failures.append(f"judge spec must use review contract version 10: {second_spec}")
...
def step_execution_plan_and_token_summary_checks(tmp: Path, run_dir: Path) -> list[str]:
    """build_execution_plan across valid/invalid/missing-gate/ready states, plus build_token_summary aggregation."""
    failures: list[str] = []
    try:
        # Written to disk by step_first_specs_and_analyst_review_checks, which always runs first.
        subagent_dir = run_dir / "subagents"
...
def build_shared_fixtures(tmp: Path, run_dir: Path) -> None:
    """Write the portfolio/chart/cache/decision-context fixtures every run_self_test step depends on."""
    portfolio = {
        "recommanded": [],
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp_name:
        ...
        build_shared_fixtures(tmp, run_dir)
        failures.extend(step_decision_brief_evidence_and_strategy_context_checks(tmp, run_dir))
        failures.extend(step_today_trade_context_checks(tmp, run_dir))
        failures.extend(step_first_specs_and_analyst_review_checks(tmp, run_dir))
        failures.extend(step_etf_and_invalid_analyst_review_checks(tmp, run_dir))
        failures.extend(step_second_spec_checks(tmp, run_dir))
        failures.extend(step_execution_plan_and_token_summary_checks(tmp, run_dir))
...
class RunSelfTestStepsAreIndividuallyDiscoverableTest(unittest.TestCase):
    """Real (non-mocked) execution of every run_self_test step_* helper, so each
    one is reachable from ordinary unittest discovery and not only from the
    mocked wrapper-orchestration test above. Steps have a genuine prerequisite
    order (each reads artifacts an earlier step wrote), so this builds that
    minimal prerequisite chain for real instead of mocking it away."""
    ...
    def setUpClass(cls) -> None:
        ...
        cls.run_dir = cls.tmp / "reports" / "runs" / "daily-trading-test"
        build_shared_fixtures(cls.tmp, cls.run_dir)
        cls.decision_brief_failures = step_decision_brief_evidence_and_strategy_context_checks(cls.tmp, cls.run_dir)
        cls.today_trade_context_failures = step_today_trade_context_checks(cls.tmp, cls.run_dir)
        cls.first_specs_and_analyst_review_failures = step_first_specs_and_analyst_review_checks(cls.tmp, cls.run_dir)
        cls.etf_and_invalid_analyst_review_failures = step_etf_and_invalid_analyst_review_checks(cls.tmp, cls.run_dir)
        cls.second_spec_failures = step_second_spec_checks(cls.tmp, cls.run_dir)
        cls.execution_plan_and_token_summary_failures = step_execution_plan_and_token_summary_checks(cls.tmp, cls.run_dir)

    def test_step_decision_brief_evidence_and_strategy_context_checks(self) -> None:
        self.assertEqual(self.decision_brief_failures, [])

    def test_step_today_trade_context_checks(self) -> None:
        self.assertEqual(self.today_trade_context_failures, [])

    def test_step_first_specs_and_analyst_review_checks(self) -> None:
        self.assertEqual(self.first_specs_and_analyst_review_failures, [])

    def test_step_etf_and_invalid_analyst_review_checks(self) -> None:
        self.assertEqual(self.etf_and_invalid_analyst_review_failures, [])

    def test_step_second_spec_checks(self) -> None:
        self.assertEqual(self.second_spec_failures, [])

    def test_step_execution_plan_and_token_summary_checks(self) -> None:
        self.assertEqual(self.execution_plan_and_token_summary_failures, [])
...
class BuildRunArtifactsSelfTest(unittest.TestCase):
    ...
    def test_self_test_suite_runs_every_step_and_reports_success(self) -> None:
        ...
        instead of re-running the whole scenario a second time."""
        step_names = [
            "step_decision_brief_evidence_and_strategy_context_checks",
            "step_today_trade_context_checks",
            "step_first_specs_and_analyst_review_checks",
            "step_etf_and_invalid_analyst_review_checks",
            "step_second_spec_checks",
            "step_execution_plan_and_token_summary_checks",
        ]
    ...
    def test_self_test_suite_reports_failure_when_a_step_fails(self) -> None:
        with patch(f"{__name__}.step_decision_brief_evidence_and_strategy_context_checks", return_value=["boom"]), patch(
            f"{__name__}.step_today_trade_context_checks", return_value=[]
        ), patch(f"{__name__}.step_first_specs_and_analyst_review_checks", return_value=[]), patch(
            f"{__name__}.step_etf_and_invalid_analyst_review_checks", return_value=[]
        ), patch(f"{__name__}.step_second_spec_checks", return_value=[]), patch(
            f"{__name__}.step_execution_plan_and_token_summary_checks", return_value=[]
        ):
            result = run_self_test()
