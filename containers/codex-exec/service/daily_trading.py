...
DAILY_TRADING_STAGE_MODEL_CONTRACT = (
    ...
    "--started-at <started_at> --env <acct|paper> --request-type <analysis|prepare|demo-submit|real-submit>`. "
    "Use CODEX_MCP_TRADING_ENV to choose acct/paper unless the explicit request requires a narrower mode.\n"
    "- After the pipeline returns, read `reports/runs/<run_id>/pipeline-summary.json` first and use it for the "
    "Telegram/user-facing response. Open `pipeline-command-log.json`, raw wrappers, `decision-brief.json`, "
    ...
    "- The routine Telegram/user-facing response must be assembled from `pipeline-summary.json` fields, not from "
    "ad hoc raw artifact rereads. Use `account_display_summary` for the main account section: 현금, 주식평가, "
    "총평가, 평가손익. Do not place `today_buy_amount` or `today_sell_amount` in the main account section; if they "
    "are relevant, show them separately as `당일 거래 누계` and do not imply they were caused by this command unless "
    "`execution.orders` confirms submitted orders. Use the exact gate label `주문 전 기존 미체결/예약 주문`, not a "
    "generic `활성 미체결/예약 주문`, so it is clear the check is a pre-submission safety gate. Include "
    "`evidence_summary.financial.display_text` and `evidence_summary.news.display_text`; distinguish missing cache "
    "from `cache_exists_zero_usable_articles`. Include the `verdict_summary`/judge-midterm result for submitted or "
    "target-changed symbols with target quantity and one-line reason when available.\n"
    "- For `analysis` or `prepare` requests, stop after the pipeline summary and do not manually rebuild the "
    "report from intermediate artifacts. For explicitly authorized `demo-submit` or `real-submit` requests, treat "
    "the pipeline `execution-plan` as a non-submitting gate summary, not as the final execution result. Open only "
    "the minimal required order/account artifacts and `trade-execution.md` gates needed to submit or block those "
    "candidates. If the execution plan says active-order or order-available gates are missing, refresh those "
    "read-only fields immediately and continue the Main-agent `order-execution` stage; do not stop just because "
    "the non-submitting plan is `partial` or `blocked`. Do not reread verdict inputs or rebuild scoring just to "
    "submit orders. For explicit limit reservation requests, treat the deterministic execution-plan `order_price` "
    ...
    "- After Main-agent order execution updates `execution.json`, run "
    "`python3 /app/skills/daily-trading/scripts/run_daily_trading_pipeline.py summarize --workspace-dir /workspace "
    "--output-dir reports/runs/<run_id> --request-type <request-type>` before the final Telegram/user-facing "
    "response, then read the refreshed `pipeline-summary.json`.\n"
