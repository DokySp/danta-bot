...
DAILY_TRADING_STAGE_MODEL_CONTRACT = (
    ...
    "- Treat `/app/skills/daily-trading/scripts/run_subagent.py` as the verified launcher interface. "
    "Do not read or paste the launcher source, persona files, or `references/rules/*.md` in full just to run "
    "collection/verdict stages; create the JSON specs and call the launcher commands below. "
    ...
    "- Runner verification command, for install/change validation only: "
    "`python3 <daily-trading-skill>/scripts/run_subagent.py self-test`.\n"
    "- first-verdict command: "
    ...
    "- second-verdict command: "
    "`python3 <daily-trading-skill>/scripts/run_subagent.py run-one --spec reports/runs/<run_id>/second-verdict-spec.json`.\n"
    "- After launcher execution, Main agent reads only `subagents/*.wrapper.json`, parsed outputs, and canonical "
    "run artifacts needed for merge/order/report. Read a stage rule file only when that stage's safety gate or "
    "schema is genuinely ambiguous.\n"
