...
def command_self_test() -> int:
    ...
    test_path = Path(__file__).resolve().parents[1] / "tests" / "test_render_touch_point.py"
    spec = importlib.util.spec_from_file_location("render_touch_point_self_test", test_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"unable to load self-test module: {test_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.command_self_test()
