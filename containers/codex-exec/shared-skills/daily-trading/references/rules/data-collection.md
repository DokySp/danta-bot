...
단일 종목 모드는 아래 순서를 모두 따른다. 다종목 포트폴리오 모드는 1~3번으로 모든 후보의 요약 종목 카드를 만들고, agent 평가 이후 4번 이후 상세 수집을 `정밀수집대상`에만 적용한다.
...
- 먼저 현재가, 등락률, 거래량, 52주 고저, 보유수량, 당일 체결/미체결/예약 상태를 수집해 1차 종목 카드를 만든다.
- 해석 불가, 현재가 누락, 거래 불가 응답처럼 agent 평가 자체가 불가능한 후보만 `제외`로 표시한다. 그 외 후보는 먼저 agent 평가 대상으로 보낸다.
- agent 평가 matrix가 나온 뒤 `정밀수집대상`과 `제외`를 나눈다. `정밀수집대상`은 추가 데이터 수집 대상이며, 주문 제출 전 계좌/API 검증 대상인 `주문검토대상`이 아니다.
- `정밀수집대상`에 대해서만 차트, 호가/체결, 수급, 재무/추정, ETF/NAV를 추가 수집한다.
- 최종 판사 row가 나온 뒤에만 `주문검토대상`을 확정한다.
