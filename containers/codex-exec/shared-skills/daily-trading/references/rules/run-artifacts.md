...
`verdict-inputs/` files are non-canonical selected-symbol slices derived from canonical artifacts for one verdict task and its `symbol_ids`. First-verdict `*.verdict-core.json` is role-scoped by the execution agent's output view input profiles; second-verdict `*.verdict-core.json` keeps the selected-symbol decision brief fields, and `*.verdict-first-slice.json` is derived from `verdict-first.json` for `second-verdict`. They reduce verdict fan-out context by excluding unrelated symbols and, for first-verdict, unrelated view fields. If a slice is missing, the canonical artifact remains the source of truth for regeneration, not direct sub-agent raw prompt fallback.

## Status Values

- `success`: required usable output exists. Optional best-effort stages may still be partial and must remain visible in `stages`.
- `partial`: usable output exists with non-fatal required symbol or required stage errors.
...
- `success`: required analysis succeeded and requested execution completed. Optional best-effort partial stages remain listed in `stages` but do not by themselves make the final run partial.
- `partial`: a usable report exists, but some required symbol/stage is partial, excluded, blocked, or failed.
...
| `verdict-inputs/*.json` | Launcher | non-canonical per-task `verdict-core` and first-verdict selected-symbol slices; first-verdict `verdict-core` is role-scoped |
...
`chart_context` is the verdict-input chart summary derived from `price-chart.json`; it must not duplicate full raw OHLCV history. Keep full raw chart rows in `price-chart.json`, and include only deterministic summaries plus a small recent-row sample in `decision-brief.json`.
