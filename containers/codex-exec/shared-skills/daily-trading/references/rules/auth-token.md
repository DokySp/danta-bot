...
- 수집 sub agent와 read-only account sub agent의 KIS 호출은 허용한다. 호출량 제한, 일시 게이트웨이 오류, timeout 같은 비인증 오류는 호출 지점에서 bounded backoff로 처리한다. `first-verdict`·`second-verdict`·`final-risk-verdict` sub agent의 KIS 호출은 금지한다.
...
4. 실패한 호출이 수집 sub agent 또는 read-only account sub agent의 호출이면, sub agent는 인증 실패를 메인 Codex에 반환하고 메인 Codex가 2~3번을 수행하게 한다. sub agent가 별도로 토큰을 재발급하거나 인증 재시도를 수행하지 않는다.
5. 재시도 후에도 실패하면 해당 데이터는 `누락`으로 표시하고, 오류 요약에 인증 실패를 남긴다.
...
- 인증 오류 후 무한 재시도
- `real`과 `demo` 토큰 혼용
- 수집 sub agent 또는 read-only account sub agent가 인증 실패 후 자체 `auth_token` 발급
