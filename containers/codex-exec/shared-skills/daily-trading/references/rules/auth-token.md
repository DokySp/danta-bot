...
검증된 `auth_token` 파라미터 템플릿을 우선 사용한다. `find_api_detail`은 새 auth API를 도입하거나, 템플릿이 없거나, API가 파라미터/schema 오류로 템플릿을 거부할 때만 호출한다.
...
4. 실패한 호출이 수집 sub agent의 호출이면, sub agent는 인증 실패를 메인 Codex에 반환하고 메인 Codex가 2~3번을 수행하게 한다. 계좌 조회는 Main agent가 직접 토큰 재발급과 단일 재시도를 수행한다.
