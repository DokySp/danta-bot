...
Strategy signals are calculated by the Main agent from the one-pass price/chart snapshot. After `price-chart.json` is finalized, no strategy tool, KIS API, web call, backtest service, or other external source may be called for any verdict stage.
