...
| 5 | `first-verdict` sub-agents | selected 4 personas | `decision-brief.json`, `verdict-format.md` | `verdict-first.json`, `verdicts/first-verdict--<agent_role>--<task_name>.md` | 외부 호출·다른 agent 결과 참조 금지; sub-agent는 compact JSON만 반환하고 companion MD는 `Main agent`가 생성 |
| 6 | `second-verdict` sub-agent | `judge-midterm` | `decision-brief.json`, `verdict-first.json`, `verdict-format.md` | `verdict-second.json`, `verdicts/second-verdict--judge-midterm--<task_name>.md` | 목표수량은 단일 judge가 제안하고 `Main agent`가 schema·자산·집중도·계좌 gate만 검증; 실패 시 `judge-midterm` 최대 2회 retry |
