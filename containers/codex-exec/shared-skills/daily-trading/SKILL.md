...
| Sub-agent verdict input | launcher-created selected-symbol slices; first-verdict slices are role-scoped by output view profiles |
...
For verdict stages, use compact specs with `artifact_paths` and `symbol_ids`; do not include `prompt`. The launcher builds the standard prompt from those fields and writes per-task `verdict-inputs/` slices containing only the listed symbols. First-verdict `verdict-core` slices are role-scoped by the execution agent's output view profiles; second-verdict also receives a selected-symbol first-verdict slice from `verdict-first.json`.
...
- `first-verdict`: selected two execution personas: `analyst-fundamental-risk` and `analyst-market-news`. `analyst-fundamental-risk` must emit two independent views, `analyst-quality-value` and `analyst-risk-allocation`; `analyst-market-news` must emit two independent views, `analyst-momentum-cycle` and `analyst-news-flow`. These merge into the canonical four first-verdict scores.
- `second-verdict`: `judge-final` only. Retry the failed `judge-final` task at most two times when the required output is missing or unusable.
...
`build_run_artifacts.py execution-plan` is the internal deterministic helper and accepts only the already-resolved `<reservation|immediate>` value. Use the launcher `run_daily_trading_pipeline.py run --order-path auto` when the order path should be selected from KST order-session time.
...
6. Use `scripts/build_run_artifacts.py first-specs`, then run the selected two `first-verdict` execution personas in parallel with launcher-created role-scoped `verdict-core` inputs. `analyst-fundamental-risk` returns separate `analyst-quality-value` and `analyst-risk-allocation` views, and `analyst-market-news` returns separate `analyst-momentum-cycle` and `analyst-news-flow` views. `analyst-news-flow` must return neutral `5` when usable news/disclosure summary is absent. Merge-first expands both wrappers back into canonical four first-verdict scores. Main agent may reduce `symbol_ids` and merge prior valid verdict rows only when it can prove price, holdings, news, and active-order status are stable; otherwise rerun the symbol. Always re-evaluate sell/stop-loss candidates, same-day fills, price shocks, new news, active orders, and score-boundary cases. Launcher automatic wrapper reuse is limited to the same spec fingerprint. Use `scripts/build_run_artifacts.py merge-first` to create `verdict-first.json` and human-review sidecars from parsed JSON.
7. Use `scripts/build_run_artifacts.py second-spec` to build the `second-verdict` set from eligible symbols with `final_first_score >= 7` plus eligible `holding` symbols from `$check-portfolio`, then run only `judge-final` with a selected-symbol first-verdict slice and at most two retries for the failed task only. Use the single valid judge target as the canonical target after deterministic validation.
