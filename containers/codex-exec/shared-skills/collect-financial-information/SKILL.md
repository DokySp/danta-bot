...
- Authentication, token, credential, and permission errors are not local backoff targets. Use the shared `kis-token` helper for OAuth token caching, return any token failure as a concise note to the daily-trading Main agent, and do not call `auth_token`.
