...
1. 입력 종목코드를 검증한다.
2. `references/rules/auth-token.md`를 읽고 요청 환경(`real` 또는 `demo`)의 KIS 접근토큰 상태를 확인한다.
...
5. `kis-trade-mcp` 도구로 필수 데이터를 수집한다. 추측한 파라미터를 쓰지 말고 도구 상세 문서에서 확인된 값을 사용한다.
...
10. 사용자가 명시적으로 모의거래 실행 또는 실전거래 주문 준비를 요청했으면 `references/rules/trade-execution.md`를 읽고 거래 처리 절차를 따른다.
...
- 사용자가 `실제 거래`, `실전 주문`, `real 주문`처럼 실전거래를 명시하면 Codex는 실제 주문 API를 제출하지 않는다. 대신 판결에 따른 예약주문 티켓, 주문 근거, 계좌·잔고·주문가능 상태, 사용자가 증권사 앱/API에서 직접 확인해야 할 최종 제출 체크리스트를 작성한다.
- 장외 시간 작업을 전제로 하므로 주문은 기본적으로 예약주문 형태로 준비한다. 모의거래 환경에서 예약주문 API가 지원되는 경우에만 실제 모의 예약주문을 제출한다.
