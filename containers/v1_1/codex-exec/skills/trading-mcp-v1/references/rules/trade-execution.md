...
이 문서는 사용자가 `/stock-trial` 이후 명시적으로 거래 처리를 요청한 경우에만 읽는다.

- 모의거래 명시 예: `모의거래로 실행`, `데모로 주문`, `demo 예약주문`
- 실전거래 명시 예: `실제 거래로 진행`, `실전 주문 준비`, `real 예약주문 검토`

분석만 요청한 경우에는 이 문서를 읽지 않고 어떤 주문도 준비하지 않는다.

## 안전 경계

- Codex는 실전거래 주문 API를 직접 제출하지 않는다.
- 실전거래 요청이 있으면 판결 기반 주문 티켓과 최종 제출 체크리스트를 작성하고, 실제 제출은 사용자가 증권사 앱 또는 별도 승인된 실행 경로에서 수행하도록 안내한다.
...
- 예약주문 API가 모의거래 환경에서 지원되지 않거나 실패하면 주문을 제출하지 말고, 동일 조건의 예약주문 티켓만 작성한다.
- 실전거래에서는 예약주문 티켓만 작성한다.
...
- 사용자가 `전액`, `올인`을 명시해도 모의거래는 최대 95%, 실전 주문 티켓은 최대 90%로 제한한다.
- 현금 완충분은 기본 5%를 남긴다. 사용자가 명시적으로 더 낮은 완충분을 지시해도 모의거래는 2% 미만, 실전 주문 티켓은 5% 미만으로 낮추지 않는다.
...
## 주문 티켓
- 환경: demo / real
- 제출 방식: 모의 예약주문 제출 / 실전 예약주문 티켓만 작성 / 주문 보류
- 종목명:
- 종목코드:
- 방향: 매수 / 매도 / 보류
- 주문 형태: 예약주문
...
사용자가 실전거래를 명시해도 Codex는 주문 API를 직접 제출하지 않는다.

1. `env_dv="real"` 기준으로 접근토큰을 확인하고 필요 시 재발급한다.
2. 계좌 자산, 잔고, 미체결, 예약주문 상태를 조회해 요약한다.
3. 매수라면 `inquire_psbl_order`로 가능수량을 확인한다.
4. 매도라면 `inquire_psbl_sell`로 가능수량을 확인한다.
5. 판결에 따른 예약주문 티켓을 작성한다.
6. 사용자가 직접 확인해야 할 최종 제출 체크리스트를 작성한다.
7. 실제 주문 제출 여부는 `제출 안 함`으로 명시한다.
